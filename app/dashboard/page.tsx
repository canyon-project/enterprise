...
export default function Dashboard() {
  ...
  return <div className={'px-[24px]'}>
    <h2 className={'py-[16px] text-[16px] text-center'} style={{fontWeight:500}}>CANYON</h2>
    <div className={'py-[48px] px-[24px]'}>
      <Space className={'pb-[32px]'}>
        <Button type={'primary'} onClick={()=>{
          setCreateModalOpen(true)
        }}>Create License Key</Button>
        <Button icon={<ExportOutlined />}>View Customer Portal</Button>
      </Space>
      {/*No License Key.*/}
      <Table columns={columns} dataSource={dataSource} pagination={false}        locale={{
        emptyText: <Empty description="No License Key."></Empty>,
      }}/>
    </div>
    <Modal title="Create License Key" open={createModalOpen} onOk={
      ()=>{
      ...
      <div className={'flex flex-col'}>
        ...
        {/*<span>{seatValue}</span>*/}
        <InputNumber style={{width:'100%'}} value={seatValue} onChange={(e)=>{
          setSeatValue(e)
        }}/>
      </div>
    </Modal>
